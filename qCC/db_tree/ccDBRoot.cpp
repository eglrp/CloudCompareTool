...
void ccDBRoot::editAttribute()
{
	...
	bool bObjDir = false;
	bool bPolyLines = false;
	CC_CLASS_ENUM  nGeotype = obj->getClassID();
	if(nGeotype == CC_TYPES::HIERARCHY_OBJECT) //ccHObject
	{
		if(obj->getChildrenNumber()<1)
			return ;
		bObjDir = true;
	}
	else if(nGeotype == CC_TYPES::POLY_LINE) //折线
	{
		bPolyLines = true;
	}
	else
	...
	QMap<QString, QVariant> mapAttriValue;
	if(!bObjDir && !bPolyLines)
	{
		QMessageBox::warning(MainWindow::TheInstance(),"waring","no suit Geometry to set");
		return ;
	}
	else
	{
		//读文本
		QFile file("../../config/ToolConfig.xml");
		ccLog::Print(file.fileName());

		if (!file.open(QIODevice::ReadOnly|QIODevice::Text))
		{
			QMessageBox::warning(MainWindow::TheInstance(),"waring","no set Attribute File");
			return ;
		}
		QString strValue = file.readAll();
		QStringList FieldsList = strValue.split("\n", QString::SkipEmptyParts);
		file.close();
		...
		//根据配置文件将字段动态加载到窗体中
		dpxAttributeEditDlg *pDlg = new dpxAttributeEditDlg(MainWindow::TheInstance());
		pDlg->setAttrFields(FieldsList);

		if(pDlg->exec() != QDialog::Accepted)
			return;

		pDlg->getAttrFields(mapAttriValue);
	}

	if(mapAttriValue.empty())
		return;
...
//设置属性
void ccDBRoot::setAttr_Recursion(ccHObject* pObj,const QMap<QString, QVariant>& VariantMap)
{
	...
	if(nSize>0)
	{
		for(int i=0;i<nSize;i++)
		{
			...
			CC_CLASS_ENUM nType = pChildObj->getClassID();
			if(nType==CC_TYPES::POINT_CLOUD || nType == CC_TYPES::GBL_SENSOR) //待补充
				continue;
