...
enum PickingOperation {	NO_PICKING_OPERATION, //不捕获
						PICKING_ROTATION_CENTER,//旋转
						PICKING_LEVEL_POINTS,//捕获点
					  };
static ccGLWindow* s_pickingWindow = nullptr; //渲染点的GLWindow
static PickingOperation s_currentPickingOperation = NO_PICKING_OPERATION;//初始化为漫游操作
...
MainWindow::MainWindow()
	...
	, m_ccRoot(nullptr) //存储数据的树形图，存储的点云图层及线
	, m_uiFrozen(false) //记录窗体是否冻住不能使用
	, m_recentFiles(new ccRecentFiles(this)) //打开的文件
	, m_3DMouseManager(nullptr)  //未编译，不用理会
	, m_gamepadManager(nullptr) //未编译，不用理会
	, m_viewModePopupButton(nullptr) //设置漫游状体
	, m_pivotVisibilityPopupButton(nullptr) //用于设置漫游中心球的大小
	, m_FirstShow(true) //判断是不是第一次显示，用于记录窗体的大小
							...
							ccGLWindow* destWin/*=0*/)
{
	...
	for ( const QString &filename : filenames )
	{
		...
		if (newGroup)
		{
			...
			{
				//disable the normals on all loaded clouds! //关闭点云的向量显示
				ccHObject::Container clouds;
