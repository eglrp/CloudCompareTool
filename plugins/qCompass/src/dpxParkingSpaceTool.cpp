...
dpxParkingSpaceTool::dpxParkingSpaceTool()
{
	m_pPickRoot = new ccHObject("ParkingSpace Lines");
...
void dpxParkingSpaceTool::toolActivated()
{
	//若添加了地图，采集到地图中去
	...
	if(pMap!=nullptr)
	{
		...
		if(pParkingSpaceLyr!=nullptr && pParkingSpaceLyr->getRootData()!=nullptr)
			m_pPickRoot = pParkingSpaceLyr->getRootData();
	}
...
//采集方法发生变化
void dpxParkingSpaceTool::slotChangeType(int nType)
{
	m_nCreateType = nType;
	if(m_nCreateType==0)
...
void dpxParkingSpaceTool::toolDisactivated()
{
...
void dpxParkingSpaceTool::setCrossWalkBorder(ccPolyline* pPoly3D,ccFitPlane* pPlane)
{
	...
	pPoly3D->setMetaData(DPX_OBJECT_TYPE_NAME,eObj_ParkingSpace); //地物类型
	pPoly3D->addChild(pPlane);
}

void dpxParkingSpaceTool::slotDeleteObj()
{
    //删除后的响应
	m_VNodeInfo.clear();
...
//右键可取消两个点时的状态
void dpxParkingSpaceTool::onMouseRightClick(int x,int y)
{
...
void dpxParkingSpaceTool::onKeyPress(int sKey)
{
	//Q键清除选择集
	if(sKey == Qt::Key_S)
	{
		...
		vector<ccPolyline*> vecLines = CreatePlaneV2(nCount,BorderPts);
		if(vecLines.size()==nCount)
		{
			for(int i = 0;i< vecLines.size();i++)
			{
				vecLines[i]->setName("ParkingSpacePon");//设置名称
...
vector<ccPolyline*> dpxParkingSpaceTool::CreatePlaneV2(int& nCount,std::vector<CCVector3>& BorderPts)
{
	...
	double dWidth = dBorderDis/nCount;
	CCVector3 vNomal;
	...
	{
		CCVector3 p0 = ptFirst 	+ dWidth*vNomal*i;
		CCVector3 p1 = ptSecond + dWidth*vNomal*i;
		CCVector3 p2 = ptThird 	+ dWidth*vNomal*i;
		CCVector3 p3 = ptFourth + dWidth*vNomal*i;
...
double dpxParkingSpaceTool::GetWidthFormBorderLine(ccPolyline* pInnerBorderLine)
{
...
void dpxParkingSpaceTool::onMouseReleaseEvent(int x,int y)
{
	if(m_window==nullptr)
		return;
	...
	*modefyPt = newPickPt;
	...
	if(bShoudAddChildInnerPon && dWidth>0 && nInnerPonCount>0)
	{
		...
		if(BorderPts.size()==4)
		{
			vector<ccPolyline*> vecLines = CreatePlaneV2(nInnerPonCount,BorderPts);
...
void dpxParkingSpaceTool::pointPicked(ccHObject* insertPoint, unsigned itemIdx, ccPointCloud* cloud, const CCVector3& P,int x/*=0*/,int y/*=0*/)
{
	if(m_nToolState)//编辑状体
		return;
...
//采集多个点 最后拟合平面
void  dpxParkingSpaceTool::PickMutilPts(const CCVector3& P ,int x,int y)
{
//if the 3D polyline doesn't exist yet, we create it
	if (!m_poly3D || !m_poly3DVertices)
	{
		...
		m_poly3D->setName("ParkingSpace Line");
...
//采集三个点 三点拟合平面
void  dpxParkingSpaceTool::ThreePtPick(const CCVector3& P ,int x,int y)
{
	//if the 3D polyline doesn't exist yet, we create it
	if (!m_poly3D || !m_poly3DVertices)
	{
